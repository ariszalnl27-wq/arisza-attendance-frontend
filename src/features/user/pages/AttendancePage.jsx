...
export default function AttendancePage() {
  ...
  // Activity modal (muncul setelah scan QR check-in berhasil dibaca)
  ...
  // ── Kirim check-in setelah user pilih activity ────────────────────
  ...
  // ── Manual check-in (buka activity modal dulu) ───────────────────
  ...
  const handleScan = (decodedText) => {
    ...
    if (scanMode === 'checkin') {
      // Tampilkan modal activity sebelum submit
    ...
    <div className="fade-in space-y-6">
      ...
      {tab === 'today' ? (
        <div className="space-y-5">
          {/* Status */}
          <div className="card">
            <div className="card-header"><p className="font-display text-base font-medium text-ink">Status Saat Ini</p></div>
            <div className="card-body"><StatusDisplay data={today} /></div>
          </div>
          ...
          {(canCheckIn || canCheckOut) && (
            <div className="card">
              <div className="card-header flex items-center justify-between">
                <p className="font-display text-base font-medium text-ink">{canCheckIn ? 'Check-in' : 'Check-out'}</p>
                <span className="text-xs text-stone-400">{canCheckIn ? 'Tiba di perpustakaan' : 'Hendak pulang'}</span>
              </div>
              <div className="card-body space-y-4">
                <button
                  className={`btn btn-lg w-full gap-3 ${canCheckIn ? 'btn-primary' : 'btn-accent'}`}
                  onClick={() => openScanner(canCheckIn ? 'checkin' : 'checkout')}
                  disabled={actionLoading}
                >
                  {actionLoading ? <Spinner size={16} /> : <CameraIcon className="w-5 h-5" />}
                  Scan QR dengan Kamera
                </button>
                <p className="text-xs text-stone-400 text-center">Arahkan kamera ke QR code yang terpasang di kasir</p>
                ...
                <div className="flex gap-2">
                  <input className="input font-mono text-xs flex-1" placeholder="Tempel token QR..."
                    value={qrToken} onChange={(e) => setQrToken(e.target.value)} />
                  <button
                    className="btn btn-secondary flex-shrink-0"
                    onClick={canCheckIn ? handleManualCheckIn : () => submitCheckOut(qrToken)}
          ...
          {!canCheckIn && !canCheckOut && today?.attendance && <InfoBanner status={today.status} />}
        </div>
      ) : (
            ...
            <>
              <div className="table-wrapper">
                <table className="table">
                  <thead><tr><th>Tanggal</th><th>Kegiatan</th><th>Check-in</th><th>Check-out</th><th>Durasi</th><th>Status</th><th>Poin</th></tr></thead>
                  <tbody>
                    {history.map((a) => (
                      <tr key={a.id}>
                        ...
                        <td>{a.point_awarded ? <span className="text-success text-xs font-medium">+1</span> : <span className="text-stone-300 text-xs">—</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <Modal open={scannerOpen} onClose={() => setScannerOpen(false)}
        title={scanMode === 'checkin' ? 'Scan QR Check-in' : 'Scan QR Check-out'} size="sm">
        <QRScanner onScan={handleScan} onClose={() => setScannerOpen(false)} />
      </Modal>

      {/* Activity Modal — muncul setelah scan QR check-in */}
      <Modal open={activityModal} onClose={() => setActivityModal(false)} title="Kegiatan Hari Ini" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-stone-600">
            QR check-in berhasil dibaca. Pilih kegiatan yang akan kamu lakukan hari ini di perpustakaan:
          </p>
          ...
          {activity === 'Lainnya' && (
            <div className="form-group">
              ...
              <input
                className="input"
                placeholder="Contoh: Membaca jurnal..."
...
function InfoBanner({ status }) {
  const map = {
    pending_approval: { bg: 'bg-amber-50 border-amber-200 text-amber-800', text: 'Check-in Anda sedang menunggu persetujuan admin.' },
    rejected:         { bg: 'bg-red-50 border-red-200 text-danger',        text: 'Check-in Anda ditolak oleh admin. Hubungi petugas.' },
    checked_out:      { bg: 'bg-stone-50 border-stone-200 text-stone-600', text: 'Kunjungan hari ini sudah selesai.' },
  }
